...
const SimpleEmailLogin = props => {
  ...
  useEffect(() => {
    if (data) {
      if (props.project && props.loginType === "payment") {
        if (!data.tokenAuth.rUser.phoneNumber) {
          props.history.push({
            pathname: "/signup",
            state: {
              projectData: props.project,
              nextPath: "/project/" + props.project.id + "/order"
            }
          });
        } else {
          props.history.push({
            pathname: "/project/" + props.project.id + "/order",
            state: { projectData: props.project }
          });
        }
      } else if (props.loginType === "saebaecoupon") {
        if (!data.tokenAuth.rUser.phoneNumber) {
          props.history.push({
            pathname: "/signup",
            state: { nextPath: "/saebaecoupon" }
          });
        } else {
          props.history.push({
